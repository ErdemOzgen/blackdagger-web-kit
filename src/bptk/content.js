...
browser.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    if (message.channel == "bptk_background2content" && message.type == "init") {
        const contentData = message
        const script = document.createElement('script')

        script.onload = () => {
            const onMessage = ({ data }) => {
                if (data.channel != 'bptk_inject2content' || !data.js) {
                    return
                }
            ...
            window.postMessage({
                channel: "bptk_content2inject",
                js: message.js,
                dom: message.dom
            })
        }

        script.setAttribute('src', browser.runtime.getURL('bptk/inject.js'))

        document.body.appendChild(script)

        return Promise.resolve()
    }
    if (message.channel == "bptk_popup2content") {
        if (message.type == "get_storage"){
            browser.runtime.sendMessage({
                channel: "bptk_content2popup",
                type: "return_storage",
...
window.addEventListener("message", (event) => {
    ...
    if (event.data?.bptk_rattacker) {
        ...
        if (event.data?.bptk_rattacker == 'start') {
        ...
        } else if (event.data?.bptk_rattacker == 'stop') {
            ...
            browser.runtime.sendMessage({
                ...
                div.id = 'bptk_rattacker_result'
                div.style.cssText = 'position:absolute;top:0px;left:0px;width:200px;height:200px;overflow:hidden';
                document.body.appendChild(div)
                //document.body.setAttribute('bptk_rattacker_result', JSON.stringify(result))

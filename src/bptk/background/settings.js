...
export class bptk_settings {
    ...
    onMessage(message, sender, sendResponse) {
        ...
        if (message.channel == "bptk_popup2background_settings") {
    ...
    async updateSettings(path, value) {
        bptk_utils.jsonSetValueByPath(this, path, value)
        return browser.storage.local.set({ "pentestkit8_settings": JSON.parse(JSON.stringify(this)) })
    }

    getSettings(path) {
        let result = this
        if (path) result = bptk_utils.jsonGetValueByPath(this, path)

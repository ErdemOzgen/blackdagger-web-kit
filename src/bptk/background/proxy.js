/* Author: ErdemOzgen */
import { bptk_logger, bptk_utils } from "./utils.js"
...
export class bptk_proxy {
    ...
    onActivated(info) {
        ...
        setTimeout(function () {
            browser.tabs.get(info.tabId).then(function (tab) {
                if (tab?.url && bptk_utils.isURL(tab?.url)) {
                    worker.bptk_app.proxy.activeTab = { tabId: tab.id, url: tab.url, window: tab.windowId }
                }
            }).catch(e => e)
        }, 150)
    }
    ...
    onBeforeRequest(request) {
        bptk_logger.log("bptk_tabs onBeforeRequest", { tabId: request.tabId, request: request.requestId, request: request })
        this.setTab(request.tabId, request, 'start')
    }

    onSendHeaders(request) {
        bptk_logger.log("bptk_tabs onSendHeaders", { tabId: request.tabId, request: request.requestId, request: request })
        this.setTab(request.tabId, request, 'request')
    }

    onBeforeRedirect(response) {
        bptk_logger.log("onBeforeRedirect", { tabId: response.tabId, request: response.requestId, response: response })
        this.setTab(response.tabId, response, 'redirect')
    }

    onCompleted(response) {
        bptk_logger.log("onCompleted", { tabId: response.tabId, request: response.requestId, response: response })
    ...
    onMessage(message, sender, sendResponse) {
        ...
        if (message.channel == "bptk_popup2background_tabs") {
    ...
    setTab(tabId, params, t) {
        if (!bptk_utils.isURL(params?.url) ) return
    ...
    getTab(tabId) {
        if (tabId in this.tabs && this.tabs[tabId] instanceof bptk_tab) return this.tabs[tabId]
        return null
    }
    ...
    updateTab(tabId, params, t) {
        ...
        try {
            if (tabId in this.tabs && this.tabs[tabId] instanceof bptk_tab) {
                this.tabs[tabId].setParams(params, t)
                bptk_logger.log("Tab updated ", { tabId: tabId })
            } else {
                this.tabs[tabId] = new bptk_tab(tabId, params, t)
                this.reduceTabs(this.maxTabsCount, tabId)
                bptk_logger.log("Tab added ", { tabId: tabId })
            }
            this.tabs[tabId].reduceTabSize(this.maxRequestsPerTab)
        } catch (e) {
            bptk_logger.log(e, "Could not update a tab", "error")
        }
    }
    ...
    set activeTab(s) {
        if (bptk_utils.isURL(this._activeTab?.url))
            this._previousTab = this._activeTab

        this._activeTab = s
        // browser.runtime.sendMessage({
        //     channel: "bptk_background2popup_tabs",
        //     type: "active tab changed"
        // }).catch(e => bptk_logger.log(e, "Could not set active tab", "warning"))
    }
    ...
    setParams(params, type) {
        if (Number.isInteger(params.frameId)) {
            ...
            if (!this.frames.has(params.frameId)) {
                ...
                bptk_logger.log("Init frames", { frameId: params.frameId, requestId: params.requestId })
            }
            ...
            if (type == 'start' || index == 0) {
                ...
                bptk_logger.log("Add new item for ", { frameId: params.frameId, requestId: params.requestId })
            } else {
                for (let p in params) {
                ...
                bptk_logger.log("Updated params ", { params: params, frameId: params.frameId, requestId: params.requestId })
            }
        } else {
            for (let p in params) {
                this[p] = params[p]
                bptk_logger.log("Add or update param ", { p: params[p] })
            }
        }
    }

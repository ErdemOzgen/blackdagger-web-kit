/* Author: ErdemOzgen */
import { bptk_logger, bptk_utils, bptk_storage, bptk_jwtHelper } from "./utils.js"
...
export class bptk_jwt {
    constructor() {
        this.storageKey = "bptk_jwt"
        this.storage = {}
    ...
    onMessage(message, sender, sendResponse) {
        ...
        if (message.channel == "bptk_popup2background_jwt") {
    ...
    async msg_init(message) {
        ...
        let activeTab = worker.bptk_app.proxy.activeTab
        let cookies = []
        if (activeTab?.tabId) cookies = await worker.bptk_app.session.getAllCookiesByTab(activeTab.tabId)
    ...
    async msg_save(message) {
        return bptk_storage.setItem(this.storageKey, { "token": message.token })
    }

    async msg_reset(message) {
        this.dict = []
        return bptk_storage.setItem(this.storageKey, {})
    }
    ...
    async getDict() {
        ...
        // else {
        let self = this
        return fetch(browser.runtime.getURL('bptk/background/jwt.secrets.list.txt'))
        ...
        for (var i = 0; i < wordlist.length; i++) {
            ...
            if ((i % 100) == 0) {
                ...
                browser.runtime.sendMessage({
                    channel: "bptk_background2popup_jwt",
                    type: "progress",
                    percent: percent,

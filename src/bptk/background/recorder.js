...
import { bptk_logger, bptk_notifications, bptk_utils, bptk_storage } from "./utils.js"
import { bptk_exporter } from "./exporter.js"

const worker = self

export class bptk_recorder {
    ...
    addListiners() {
        ...
        browser.webRequest.onBeforeRequest.addListener(this.onBeforeRequest, { urls: ["<all_urls>"], types: bptk_utils.requestFilters },
            ["requestBody"].concat(bptk_utils.extraInfoSpec)
        )

        this.onSendHeaders = this.onSendHeaders.bind(this)
        browser.webRequest.onSendHeaders.addListener(this.onSendHeaders, { urls: ["<all_urls>"], types: bptk_utils.requestFilters },
            ["requestHeaders"].concat(bptk_utils.extraInfoSpec)
        )

        this.onHeadersReceived = this.onHeadersReceived.bind(this)
        browser.webRequest.onHeadersReceived.addListener(this.onHeadersReceived, { urls: ["<all_urls>"], types: bptk_utils.requestFilters },
            ["responseHeaders"].concat(bptk_utils.extraInfoSpec)
        )

        this.onBeforeRedirect = this.onBeforeRedirect.bind(this)
        browser.webRequest.onBeforeRedirect.addListener(this.onBeforeRedirect, { urls: ["<all_urls>"], types: bptk_utils.requestFilters },
            ["responseHeaders"].concat(bptk_utils.extraInfoSpec)
        )

        this.onCompleted = this.onCompleted.bind(this)
        browser.webRequest.onCompleted.addListener(this.onCompleted, { urls: ["<all_urls>"], types: bptk_utils.requestFilters },
            ["responseHeaders"].concat(bptk_utils.extraInfoSpec)
        )
    ...
    onBeforeRequest(request) {
        if (bptk_utils.exclude(request.url) || request.type.match(/(ping)/) || !this.isTracking(request.tabId)) return
        ...
        if (this.mode == "recording") {
            ...
            catch (e) { e => bptk_logger(e, "Could not update recording request", "warning") }
        }
    }
    ...
    onMessage(message, sender) {
        ...
        if (!bptk_utils.isTrustedOrigin(sender))
            return Promise.reject({ success: false, error: 'Error origin value' })

        if (message.channel == "bptk_popup2background_recorder") {
    ...
    msg_save_macro(message) {
        ...
        bptk_storage.setItem(this.storageKey, this.storage)
    ...
    msg_export_recording(message) {
        let exporter = new bptk_exporter(this.recording, message.settings)
    ...
    startRecording(cleanCookie, startUrl, bootstrap) {
        if (this.mode == null) {
            ...
            worker.bptk_recorder_active = true
            this.mode = 'recording'
            ...
            browser.storage.local.set({
                ...
                "bptk_recording_timing": [],
                "bptk_recording": { mode: "recording", startUrl: startUrl },
                "bptk_recording_log": "",
                "bptk_recording_confirm_required": true,
                "bptk_path_to_icons": this.pathToIcons,
                "bptk_double_click": this.doubleClick
            }).then(function () {
            ...
            bptk_notifications.notify("Recording/playback already started", "Stop recording before start a new one");
        }
    }

    stopRecording(params) {
        worker.bptk_recorder_active = false
        this.mode = null
        this.openerWinId = -1
        this.openerTabId = -1
        this.tabs = []
        ...
        browser.storage.local.get(["bptk_recording_items", "bptk_recording_timing"]).then(async function (result) {
            if (!result) return

            let a = this.recording.requests
            let b = result.bptk_recording_timing
            ...
            this.recording.items = result.bptk_recording_items
            ...
            await bptk_storage.setItem(this.storageKey, this.storage)
            browser.runtime.sendMessage({
                channel: "bptk_background2popup_recorder",
                type: "recording_completed",
                recording: JSON.parse(JSON.stringify(this.recording))
            }).catch(e => bptk_logger.log(e, "Could send recording completed", "warning"))
    ...
    startReplay(cleanCookie, startUrl, items, validateRegex) {
        if (this.mode == null) {
            ...
            return browser.storage.local.set({
                "bptk_replay_items": items,
                "bptk_replay_step": 0,
                "bptk_replay_regex": validateRegex,
                "bptk_replay": { mode: "replay", startUrl: startUrl },
                "bptk_recording_log": "",
                "bptk_recording_confirm_required": true,
                "bptk_path_to_icons": this.pathToIcons
            }).then(function () {
    ...
    reset() {
        ...
        delete worker.bptk_recorder_active
        browser.storage.local.remove(
            [
                "bptk_recording",
                "bptk_recording_items",

...
export class bptk_sca {
    constructor() {
        ...
        this.storageKey = "bptk_sca"
        this.storage = bptk_storage.getItem(this.storageKey)
    ...
    onMessage(message, sender, sendResponse) {
        ...
        if (message.channel == "bptk_popup2background_sca") {
    ...
    async msg_init(message) {
        ...
        return Promise.resolve(Object.assign({}, this.storage, worker.bptk_app.proxy.activeTab))
    }

    async msg_reset(message) {
        this.storage = bptk_storage.setItem(this.storageKey, {})
        return Promise.resolve(Object.assign({}, worker.bptk_app.proxy.activeTab))
    }
    ...
    async msg_scan(message) {
        ...
        if (tab) {
            return tab?.analyze().then(async function (result) {
                ...
                bptk_storage.setItem(self.storageKey, { scan_result: scan_result })
                return Promise.resolve(Object.assign({}, { scan_result: scan_result }, worker.bptk_app.proxy.activeTab))
            })
        }
    }
    ...
    async msg_save(message) {
        bptk_storage.setItem(this.storageKey, message.scan)
    }



    async initRepo() {
        await fetch(browser.runtime.getURL('bptk/packages/retire/jsrepository.json'))

/* Author: ErdemOzgen */
import { bptk_utils, bptk_storage, bptk_ruleManager, bptk_logger } from "./utils.js"
import { httpZ } from "./lib/httpZ.esm.js"

const worker = self

export class bptk_request_manager {

    constructor(settings) {
        this.storageKey = 'bptk_rbuilder'
        this.storage = []
    ...
    onMessage(message, sender, sendResponse) {
        ...
        if (message.channel == "bptk_popup2background_request") {
    ...
    async msg_parse_request(message) {
        let rbObj = bptk_request.parseRawRequest(message.raw, message.opts)
        if (message.formId) {
    ...
    async msg_update_request(message) {
        let rbObj = bptk_request.updateRawRequest(message.schema, message.params, message.opts)
        if (message.formId) {
            if (!this.storage[message.formId]) {
                this.storage[message.formId] = rbObj
                this.storage[message.formId].sort = Object.keys(this.storage).length
    ...
    async msg_delete_request(message) {
        if (this.storage[message.formId]) {
            ...
            await bptk_storage.setItem(this.storageKey, this.resort(this.storage))
        }
        return Promise.resolve(JSON.parse(JSON.stringify(this.storage)))
    }
    ...
    async msg_send_request(message) {
        ...
        return request.sendRequest(message.schema).then(function (response) {
            if (message.formId) {
                ...
                bptk_storage.setItem(self.storageKey, self.storage)
            }
    ...
    async msg_scan_request(message) {
        worker.bptk_app.rattacker.scanRequest(message.schema.request.raw).then(scanResult => {
            let request_manager = worker.bptk_app.request_manager
            if (request_manager.storage[message.formId]) {
                ...
                bptk_storage.setItem(request_manager.storageKey, request_manager.storage)
            }
            browser.runtime.sendMessage({
                channel: "bptk_background2popup_rbuilder",
                type: "scan completed",
...
export class bptk_request {
    ...
    addListeners() {
        ...
        browser.webRequest.onBeforeRequest.addListener(
            ...
            ["requestBody"].concat(bptk_utils.extraInfoSpec).concat(blocking)
        )

        this.onBeforeSendHeaders = this.onBeforeSendHeaders.bind(this)
        browser.webRequest.onBeforeSendHeaders.addListener(
            this.onBeforeSendHeaders,
            { urls: ["<all_urls>"], types: bptk_utils.filterType },
            ["requestHeaders"].concat(bptk_utils.extraInfoSpec).concat(blocking)
        )
        ...
        browser.webRequest.onHeadersReceived.addListener(
            ...
            { urls: ["<all_urls>"], types: bptk_utils.filterType },
            ["responseHeaders"].concat(bptk_utils.extraInfoSpec).concat(blocking)
        )
    ...
    static updateRawRequest(schema, params, opts) {
        ...
        bptk_request.normalizeHeaders(schema, opts)
        schema.request.raw = httpZ.build(schema.request, opts)
        return schema
    }

    static parseRawRequest(raw, opts) {
        let schema = bptk_request.rbuilderScheme()
        if (!opts) opts = schema.opts
        else schema.opts = opts

        schema.request = Object.assign(httpZ.parse(bptk_request.getParsedRaw(raw), opts))
    ...
    async sendRequest(schema) {
        this.addListeners()
        // bptk_ruleManager.getDynamicRules()
        // bptk_ruleManager.getSessionRules()
        let ruleId = null
        ...
        if (schema.opts.override_headers != false && schema.request.headers.length > 0) {
            ...
            await bptk_ruleManager.addSessionRule(schema, ruleId)
        ...
        return fetch(schema.request.url, params).then(async function (response) {
            ...
            if (ruleId) {
                bptk_ruleManager.removeSessionRule(ruleId)
            }

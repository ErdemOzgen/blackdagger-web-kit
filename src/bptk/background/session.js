/* Author: ErdemOzgen */
import { bptk_utils, bptk_storage } from "../background/utils.js"

const worker = self

export class bptk_session {

    constructor(settings) {
        this.settings = settings
        this.storageKey = "bptk_cookies"
    ...
    onMessage(message, sender, sendResponse) {
        ...
        if (message.channel == "bptk_popup2background_session") {
    ...
    buildCookieUrl(cookie) {
        if (!cookie.secure && worker.bptk_app.proxy?.activeTab)
            cookie.secure = worker.bptk_app.proxy.activeTab.url.indexOf("https://") === 0
    ...
    async getAllCookiesByTab(tabId) {
        let promises = []
        let tab = worker.bptk_app.proxy.getTab(tabId)

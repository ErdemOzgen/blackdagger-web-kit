  ...
  <MacroEvent>
    ...
    javascriptGetMacroItem(path, attrs) {
        ...
        if (attrs.length > 0 && path == "") {
            if (attrs[0] == 'a') {
                output += `let item = Array.prototype.slice.call(document.querySelectorAll('bptk_ATTRNAME_bptk')).filter(function (el) { return el.textContent === 'bptk_ATTRVALUE_bptk'})[0];
                `.replace(/bptk_ATTRNAME_bptk/g, attrs[0]).replace(/bptk_ATTRVALUE_bptk/g, attrs[1]);;
            } else {
                path = '[' + attrs[0] + ' = "' + attrs[1] + '"]';
                output += `let item = document.querySelector('bptk_PATH_bptk'); `.replace(/bptk_PATH_bptk/g, path);
            }
        } else {

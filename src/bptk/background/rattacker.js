/* Author: ErdemOzgen */
import { bptk_module } from "../modules/module.js"
import { bptk_request } from "./rbuilder.js"
import { bptk_utils, bptk_logger, bptk_queue, bptk_storage, bptk_ruleManager } from "../background/utils.js"

const worker = self

export class bptk_rattacker {

    constructor(settings) {
        this.settings = settings
        this.storageKey = "bptk_rattacker"
    ...
    async loadModules() {
        ...
        let self = this
        await fetch(browser.runtime.getURL('bptk/modules/modules.json'))
            .then(response => response.json())
            .then(json => {
                Object.values(json.modules).forEach(module => {
                    self.modules.push(new bptk_module(module))
                })
            })
    }
    ...
    async loadProModules() {
        // let self = this
        // this.pro_modules = []
        // let apiKey = worker.bptk_app?.settings?.profile?.api_key
        // let url = worker.bptk_app.settings.profile.api_url + worker.bptk_app.settings.profile.attacks_endpoint
        // if (apiKey) {
        ...
        //                 self.pro_modules.push(new bptk_module(module))
        //             })
        //         }).catch(e => {
        //             console.log(e)
    ...
    async init() {
        ...
        if (!this.isScanRunning) {
            this.storage = await bptk_storage.getItem(this.storageKey)
            if (Object.keys(this.storage).length > 0) {
                this.scanResult = this.storage
                this.scanResult.uniqueRequestQueue = new bptk_queue(this.scanResult.uniqueRequestQueue.items)
                this.scanResult.requestQueue = new bptk_queue(this.scanResult.requestQueue.items)
            }
        }
    }
    ...
    getScanResultSchema() {
        return {
            ...
            tabId: null,
            host: null,
            uniqueRequestQueue: new bptk_queue(),
            requestQueue: new bptk_queue(),
            items: [],
            stats: {
                vulnsCount: 0,
                high: 0,
    ...
    async reset() {
        bptk_storage.setItem(this.storageKey, {})
    ...
    addListeners() {
        ...
        browser.webRequest.onCompleted.addListener(
            this.onCompleted,
            { urls: ["<all_urls>"], types: bptk_utils.requestFilters },
            ["responseHeaders"].concat(bptk_utils.extraInfoSpec)
        )
    }
    ...
    onCompleted(response) {
        ...
        if (this.isLegalUrl(response.url)) {
            if (!this.settings.blacklist.includes(response.type) || s.search) {
                try {
                    let r = worker.bptk_app.proxy.getRawRequest(worker.bptk_app.proxy.getTab(response.tabId), response.frameId, response.requestId)
                    this.addRequest(r)
                } catch (e) { }
            }
        }
    ...
    onMessage(message, sender, sendResponse) {
        ...
        if (message.channel == "bptk_popup2background_rattacker") {
        ...
        if (message.channel == "bptk_content2rattacker") {
    ...
    async msg_init(message) {
        ...
        return Promise.resolve({
            ...
            activeTab: worker.bptk_app.proxy.activeTab
        })
    }
    ...
    async msg_save_report(message) {
        ...
        if (apiKey && Object.keys(this.scanResult?.items)) {
            let url = worker.bptk_app.settings.profile.api_url + worker.bptk_app.settings.profile.storage_endpoint
    ...
    async msg_download_scans(message) {
        let apiKey = worker.bptk_app.settings.profile?.api_key
        if (apiKey) {
            let url = worker.bptk_app.settings.profile.api_url + worker.bptk_app.settings.profile.scans_endpoint
            let response = await fetch(url, {
                headers: {
                    'Authorization': apiKey,
                },
    ...
    async msg_download_scan_by_id(message) {
        let apiKey = worker.bptk_app.settings.profile?.api_key
        if (apiKey) {
            let url = worker.bptk_app.settings.profile.api_url + worker.bptk_app.settings.profile.scans_endpoint + "/" + message.scanId
            let response = await fetch(url, {
                headers: {
                    'Authorization': apiKey,
                },
                ...
                .then(response => response.json())
                .then(json => {
                    bptk_storage.setItem(this.storageKey, json)
                    return json
                }).catch(e => e)
            return response
        }
    ...
    async msg_delete_scan_by_id(message) {
        let apiKey = worker.bptk_app.settings.profile?.api_key
        if (apiKey) {
            let url = worker.bptk_app.settings.profile.api_url + worker.bptk_app.settings.profile.storage_endpoint + "/" + message.scanId
            let response = await fetch(url, {
                method: "DELETE",
                headers: {
                    'Authorization': apiKey,
    ...
    async msg_save(message) {
        this.reset()
        bptk_storage.setItem(this.storageKey, JSON.parse(message.json))
        await this.init()
        return Promise.resolve({
            scanResult: JSON.parse(JSON.stringify(this.scanResult)),
            isScanRunning: this.isScanRunning,
    ...
    stopBackroungScan() {
        ...
        bptk_storage.setItem(this.storageKey, this.scanResult)
    ...
    updateScanResult(result, data) {
        ...
        for (let r in this.scanResult.items) {
            ...
            if (data) {
                let i = item.attacks.findIndex(a => (a.metadata.action?.random == data.attackValue.bptk && !a.success))
                if (i > -1) {
                    item.attacks[i].success = true
                    item.attacks[i].proof = 'Confirmed by code execution on ' + data.location + '. Attack parameter value is: ' + data.attackValue.bptk
                }
            }
        ...
        if (this.scanResult.requestQueue.size() /*&& self.scanningRequest == false*/) {
            ...
            this.scanRequest(item).then(function (result) {
                ...
                browser.runtime.sendMessage({
                    channel: "bptk_background2popup_rattacker",
                    type: "all attacks completed",
                    info: item,
                    scanResult: JSON.parse(JSON.stringify(self.scanResult))
                }).catch(e => bptk_logger.log(e, "Could not send a message", "info"))
    ...
    async scanRequest(raw) {
        let schema = bptk_request.parseRawRequest(raw)
        ...
        for (let key in modules) {
            ...
            for (let attackIndex in module.attacks) {
                ...
                if (module.type == 'active') {
                    let _schema = bptk_request.parseRawRequest(original.request.raw, attack.action.options)
                    let attackRequests = module.buildAttacks(_schema, attack)

                    for (let i in attackRequests) {
                        //attackRequests[i].request.target += attackRequests[i].request.queryParams.length > 0 ? "" : "?" + "&bptk_attack=" + attack.id
                        let _s = bptk_request.updateRawRequest(attackRequests[i], null, attack.action.options)
    ...
    //             if (module.type == 'active') {
    //                 let _schema = bptk_request.parseRawRequest(raw, attack.action.options)
    //                 let attackRequests = module.buildAttacks(_schema, attack)
    //                 for (let i in attackRequests) {
    //                     try {
    //                         bptk_request.normalizeHeaders(attackRequests[i], attack.action.options)
    ...
    async activeAttack(schema) {
        ...
        try {
            let request = new bptk_request()
            return request.sendRequest(schema)
        } catch (error) {
            bptk_logger.log(error, "Could not run an attack", "info")
        }

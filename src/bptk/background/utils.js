...
import { bptk_decoder } from "./decoder.js"

/* Utils */
const worker = self


export class bptk_utils {
    constructor() { }
...
/* Logger */
export class bptk_logger {
    constructor() { }

    static log(event, msg, level) {
        // if (window.bptk_debug || level == "error") {
        //     console.log(event)
...
export class bptk_notifications {
    constructor() { }
...
export class bptk_jwtHelper {
    ...
    detectCertFormat(cert) {
        if (typeof cert == 'string') {
            if (cert.startsWith("-----BEGIN PUBLIC KEY-----")) return bptk_jwtHelper.SPKI
            if (cert.startsWith("-----BEGIN PRIVATE KEY-----")) return bptk_jwtHelper.PKCS8
            try {
                JSON.parse(cert)
                return bptk_jwtHelper.JWK
            } catch (e) {
                return null
            }
        }
    ...
    async signToken(header, payload, secret, keys) {
        ...
        if (hObj.alg.toLowerCase() == 'none') {
            ...
            } else if (format == bptk_jwtHelper.JWK) {

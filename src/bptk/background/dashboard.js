...
import { bptk_utils } from "./utils.js"


const worker = self

export class bptk_dashboard {
    constructor() {

        fetch(browser.runtime.getURL('bptk/packages/wappalyzer/technologies.json'))
        ...
        fetch(browser.runtime.getURL('bptk/packages/wappalyzer/waf.json'))
            .then(response => response.json())
            .then(data => {
    ...
    async initCookies(urls) {
        ...
        return Promise.all(promises).then(function (cookie) {
            ...
            browser.runtime.sendMessage({
                channel: "bptk_background2popup_dashboard",
                type: "init_complete",
    ...
    async msg_get(message) {
        return Promise.resolve(Object.assign({},
            this,
            worker.bptk_app.proxy.activeTab))
    }
    ...
    async msg_init(message) {
        if (worker.bptk_app?.settings?.history?.route != 'index') {
            let link = ""
            if (['session', 'sca', 'proxy', 'rbuilder', 'rattacker', 'recording', 'decoder', 'swagger-editor', 'portscanner', 'jwt', 'xss','sql'].includes(worker.bptk_app.settings.history.route)) {
                link = worker.bptk_app.settings.history.route + ".html"
                if (worker.bptk_app.settings.history.hash) {
                    link += "#" + worker.bptk_app.settings.history.hash
                }
            }
            if (link != "")
        ...
        this.activeTab = worker.bptk_app.proxy.activeTab

        this.privacy = worker.bptk_app.settings.privacy
        ...
        if (this.activeTab?.tabId) {
            ...
            browser.tabs.sendMessage(this.activeTab.tabId, {
                channel: "bptk_background2content",
                type: "init",
                dom: this.wappalyzerDomRules,
                js: this.wappalyzerJsRules
            }).catch(() => { })

            let tab = worker.bptk_app.proxy.getTab(this.activeTab.tabId)
            let self = this
            if (tab) {
                return tab?.analyze().then(function (result) {
                    ...
                    return Promise.resolve(Object.assign({}, self, worker.bptk_app.proxy.activeTab, { findings: self.tab.findings }))
                })
            }
        }
        return Promise.resolve(Object.assign({}, worker.bptk_app.proxy.activeTab, { privacy: this.privacy }))
    }

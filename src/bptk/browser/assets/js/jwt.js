...
import { bptk_controller_jwt } from "../../../controller/jwt.js"
import { bptk_controller_session } from "../../../controller/session.js"


const controller = new bptk_controller_jwt()
const session_controller = new bptk_controller_session()
...
jQuery(function () {
    ...
    async function generateConfusion(header, payload, secret) {
        ...
        } else if (format == bptk_jwtHelper.SPKI) {
            publicKey = secret
        }
    ...
    $("#dlg_btn_key_download").on('click', function (e) {
        ...
        publicJWK.kid = "bptk_jwt_inspector"
        privateJWK.kid = "bptk_jwt_inspector"
    ...
    $("#dlg_btn_key_generate").on('click', function (e) {
        ...
        if ($form.form('is valid')) {
            ...
            generateKeyPair(values['generate_keys_dlg_algorithm']).then(result => {
                result['jwk_public_key']['kid'] = 'bptk_jwt_inspector'
                result['jwk_private_key']['kid'] = 'bptk_jwt_inspector'
    ...
    $("#pem_jwk_convertor_pem_btn").on('click', function (e) {
        ...
        if ($pemform.form('is valid')) {
            ...
            if (format == bptk_jwtHelper.PKCS8) {
...
browser.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    ...
    if (message.channel == "bptk_content2popup" && message.type == "return_storage") {
        checkStorage(message.data)
        bindTokens(tokens)
    }

    if (message.channel == "bptk_background2popup_jwt") {

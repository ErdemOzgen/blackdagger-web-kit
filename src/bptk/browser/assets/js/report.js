/* Author: ErdemOzgen */
import { bptk_controller_index } from "../../../controller/index.js"
import { bptk_controller_sca } from "../../../controller/sca.js"
import { bptk_controller_rattacker } from "../../../controller/rattacker.js"
import { bptk_utils, bptk_jwtHelper } from "../../../background/utils.js"
import { bptk_decoder } from "../../../background/decoder.js"
import * as rutils from "../js/rutils.js"

const jwtHelper = new bptk_jwtHelper()
const decoder = new bptk_decoder()
...
jQuery(function () {
    // -- Dashboard -- //
    const index_controller = new bptk_controller_index()
    const sca_controller = new bptk_controller_sca()
    const rattacker_controller = new bptk_controller_rattacker()
    ...
    async function bindInfo(host) {
        if (host) {
            $('#dashboard_message_text').html('<h2>OWASP BPTK report:</h2>  ' + host)
        } else {
    ...
    function bindReportItem(info, original) {
        ...
        if (info.proof)
            proof = `<div class="description"><p>Proof: <b><i name="proof">${bptk_utils.escapeHtml((info.proof))}</i></b></p></div>`
        ...
        let item = `<div class="attack_info ${attackClass} ui segment">
                        <div class="ui ${color} message" style="margin-bottom: 0px;">
                            <div class="content">
                                ...
                                <p>Attack: ${bptk_utils.escapeHtml(info.metadata.name)} </p>
                                ${proof}
                            </div>
                        </div>
                    <div class="two fields" >
                        <div class="one field">
                            <textarea class="codemirror_area" style="width:100%;  border: solid 1px #cecece; padding: 1px;">${bptk_utils.escapeHtml(request)}</textarea>
                        </div>
                        <div class="one field">
                            <textarea class="codemirror_area_html" style="width:100%;  border: solid 1px #cecece; padding: 1px;">${bptk_utils.escapeHtml(response)}</textarea>
                        </div>
                    </div></div>`

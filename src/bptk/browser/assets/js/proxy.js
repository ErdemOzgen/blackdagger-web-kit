/* Author: ErdemOzgen */
import { bptk_controller_proxy } from "../../../controller/proxy.js"
const controller = new bptk_controller_proxy()
...
jQuery(function () {
    ...
    $(document).on("click", "#download_domain_list", function () {
        ...
        if (data != null) {
            ...
            downloadLink.download = "BPTK_domains_" + (new URL(controller.url)).hostname + ".txt"
    ...
    $(document).on("click", "#download_url_list", function () {
        ...
        if (data != null) {
            let blob = new Blob([data.join("\r\n")], { type: 'text/plain' })
            let downloadLink = document.createElement("a")
            downloadLink.download = "BPTK_urls_" + (new URL(controller.url)).hostname + ".txt"
            downloadLink.href = window.URL.createObjectURL(blob)
            downloadLink.click()
        }
    })
...
browser.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    ...
    if (message.channel == "bptk_background2popup_tabs") {

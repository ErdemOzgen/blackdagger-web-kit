...
import { bptk_controller_macro } from "../../../controller/macro.js"
const controller = new bptk_controller_macro()
        ...
        try {
            ...
            if ($events.length > 0) {
                setTimeout(function (firstEvent) {
                    ...
                    let fName = "BPTK_macro.rec"
                    let url = $(firstEvent).find('Data').text()

                    if (url) {
                        fName = 'BPTK_' + (new URL(url)).hostname + '.rec'
                    }
...
browser.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    if (message.channel == "bptk_background2popup_recorder" && message.type == "recording_completed") {

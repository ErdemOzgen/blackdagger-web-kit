...
export class bptk_module {
    ...
    modifyProps(schema, action) {
        for (let i = 0; i < action.props.length; i++) {
            bptk_utils.jsonSetValueByPath(schema, action.props[i].name, action.props[i].value, true)
        }
        return schema
    }
    ...
    modifyPostParams(schema, action) {
        ...
        if (params) {
            ...
            params.push({ name: 'bptk_rnd', value: bptk_utils.attackParamId() })
        }
        return schema
    }
    ...
    prepareAttack(a) {
        ...
        let rnd = bptk_utils.attackParamId()

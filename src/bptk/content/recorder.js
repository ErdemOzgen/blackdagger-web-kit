...
(function () {
    if (window.bptk_recorder || typeof browser === typeof undefined) return
                ...
                if (parentElement && parentElement.querySelectorAll(':scope > ' + element.nodeName).length > 1) {
                    ...
                    for (let i = 0; i < list.length; i++) {
                        if (list[i].isSameNode(element)) {
                            index = i
                            if (element.getAttribute('BPTK_originalIndex') && parseInt(element.getAttribute('BPTK_originalIndex')) > 0) {
                                index = parseInt(element.getAttribute('BPTK_originalIndex'))
                            }
                            break
                        }
                    }
    ...
    class bptk_event_click extends bptk_event {
        constructor(e) {
            super(e)
    ...
    class bptk_event_dblclick extends bptk_event {
        constructor(e) {
            super(e)
    ...
    class bptk_event_keypress extends bptk_event {
        constructor(e) {
            super(e)
    ...
    class bptk_event_setwindowsize {
        constructor(e) {
            this.eventStart = (new Date()).getTime()
    ...
    class bptk_event_change extends bptk_event {
        constructor(e) {
            super(e)
    ...
    class bptk_testcase {
        ...
        append(item) {
            if (item?.props?.id?.startsWith('bptk_recording_')) return
            let prev = {}
        ...
        sync(items, log) {
            ...
            if (log) this.log = log
            return browser.storage.local.set({ "bptk_recording_items": this.items, "bptk_recording_log": this.log })
        }
    ...
    class bptk_recorder {
        constructor() {
            ...
            this.testcase = new bptk_testcase()

            browser.storage.local.get(['bptk_recording', 'bptk_recording_items', 'bptk_recording_timing', 'bptk_recording_log', 'bptk_double_click']).then(function (result) {

                this.doubleClick = result.bptk_double_click
                if (!isIframe) {
                    this.testcase = new bptk_testcase(result.bptk_recording_items, result.bptk_recording_log)
                    if (this.testcase.items.length == 0) {
                        let evtNavigate = new bptk_event_navigate(result.bptk_recording?.startUrl)
                    ...
                    setTimeout(function () {
                        if (this.testcase.peek().eventType != EventTypes.Delay) {
                            let evtDelay = new bptk_event_delay()
            ...
            this.mutationObserver = new MutationObserver(function (mutations) {
                mutations.forEach(function (mutation) {
                    if (mutation.target.tagName == 'HEAD' || mutation.target.tagName == 'SCRIPT' || mutation.target.tagName == 'STYLE' ||
                        (mutation.target.id && mutation.target.id.toLowerCase().startsWith('bptk_')) ||
                    ...
                    if (mutation.type == 'childList' && mutation.addedNodes.length > 0) {
                        ...
                        for (let i = 0; i < mutation.target.children.length; i++) {
                            if (mutation.target.children[i].tagName == addedNode.tagName) {
                                if (!mutation.target.children[i].getAttribute("BPTK_originalIndex"))
                                    mutation.target.children[i].setAttribute("BPTK_originalIndex", count)
                                count++
                            }
                        }
                    }
                    if (mutation.type == 'childList' && mutation.removedNodes.length > 0) {
                        ...
                        let count = 0
                        for (let i = 0; i < mutation.target.children.length; i++) {
                            if (mutation.target.children[i].tagName == removedNode.tagName && mutation.target.children[i].getAttribute("BPTK_originalIndex")) {
                                count = parseInt(mutation.target.children[i].getAttribute("BPTK_originalIndex"))
                                if (count > -1) mutation.target.children[i].setAttribute("BPTK_originalIndex", (count - 1))
                            }
                        }
                    }
                })
        ...
        clickaction(evt) {
            ...
            if (evt.elementType == "checkbox" || evt.elementType == "radio") {
                elData = new bptk_event_keypress(evt.event)
            }
        ...
        onclick(e) {
            ...
            let evt = new bptk_event_click(e)
            if (e?.srcElement?.id?.startsWith('bptk_recording') || evt.xpath == "/") return false
        ...
        ondblclick(e) {
            ...
            let evt = new bptk_event_dblclick(e)
            if (e?.srcElement?.id?.startsWith('bptk_recording') || evt.xpath == "/") return false
        ...
        oncustomevent(e) {
            if (e?.srcElement?.id?.startsWith('bptk_recording'))
                return false
            let evt = new bptk_event_click(e)
        ...
        onkeypress(e) {
            ...
            let evt = new bptk_event_keypress(e)
    ...
    if (!window.bptk_recorder) window.bptk_recorder = new bptk_recorder()

    window.addEventListener("message", (event) => {
        if (!isIframe && event.data.channel == 'sync') {
            if (window.bptk_recorder?.testcase) window.bptk_recorder.testcase.sync(event.data.items, event.data.log)
        }
        else if (isIframe && event.data.channel == 'frameInfo') {
            frameInfo = event.data.item
            window.bptk_recorder.testcase = new bptk_testcase(event.data.testcase.items, event.data.testcase.log)
        }
    })

    browser.storage.onChanged.addListener(function (changes, namespace) {
        if (changes['bptk_recording_items']) {
            if (!window.bptk_recorder)
                window.bptk_recorder = new bptk_recorder()

            window.bptk_recorder.testcase.items = changes['bptk_recording_items'].newValue
        }

        if (changes['bptk_recording_log']) {
            window.bptk_recorder.testcase.log = changes['bptk_recording_log'].newValue
        }
    })

    setTimeout(function () {
        browser.storage.local.get("bptk_recording_timing").then(function (result) {
            let entries = performance.getEntries().filter(v => ['navigation', 'resource'].includes(v.entryType))
            let storage = result.bptk_recording_timing
            let s = new Set([...storage, ...entries])

            browser.storage.local.set({
                "bptk_recording_timing": JSON.parse(JSON.stringify(Array.from(s)))
            })
        })
    }, isIframe ? 20 : 0)

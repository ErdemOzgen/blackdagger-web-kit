...
(function () {
    if (window.bptk_replayer || typeof browser === typeof undefined) return
    ...
    class bptk_replayer {
        constructor() {
            browser.storage.local.get([
                ...
                'bptk_replay_step',
                'bptk_replay_regex',
                'bptk_recording_log']).then(function (result) {
                    if (result.bptk_replay_step == -1) return
                    this.items = result.bptk_replay_items
                    this.step = result.bptk_replay_step
                    this.regex = result.bptk_replay_regex
                    this.paused = false
                    this.forward = false
                    this.log = result.bptk_recording_log
        ...
        async start() {
            if (this.step > -1) {
                while (this.step < this.items.length) {
                    ...
                    this.logEvent(item)
                    browser.storage.local.set({ 'bptk_replay_step': (this.step + 1) })
        ...
        stop() {
            this.step = -1
            browser.storage.local.set({ "bptk_replay_step": this.step })
        }
